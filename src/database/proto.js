...
export default class Database {
  ...
  login() {
    ...
    Object.keys(this.schema.authDataSchema).forEach((key) => {
      const data = localStorage.getItem(`DB-${key}`) || undefined
      ...
      try {
        ...
        console.log('The data structure is incorrect.')
        console.error(err)
      }
    })
  }
  ...
  reflash(table) {
    if (table) {
      ...
      if (dataDB) {
        localStorage.setItem(`DB-${table}`, JSON.stringify(dataDB.cache))
      }
    } else {
      Object.keys(this.data).forEach((key) => {
        const dataDB = this.data[key]
        if (dataDB) {
          localStorage.setItem(`DB-${key}`, JSON.stringify(dataDB.cache))
        }
      })
    }
  }
